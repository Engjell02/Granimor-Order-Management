...
export default function Create() {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (!fullName.trim() || !phoneNumber.trim() || !location.trim() || !dueDate || !hasValidItems) return;
  ...
  return (
    <div style={{ maxWidth: 780, margin: '0 auto' }}>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 26, fontWeight: 700, color: '#1a1a2e' }}>{t.createOrder}</h1>
        <p style={{ color: '#888', marginTop: 4 }}>{t.createOrderSub}</p>
      </div>
      <form onSubmit={handleSubmit}>

        {/* Client Info */}
        <div className="card">
          <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 20 }}>{t.clientInfo}</div>
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <input value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)} placeholder="+389 7X XXX XXX" required />
            </div>
          </div>
          <div className="form-row">
            <div className="form-group">
              <label>{t.location} *</label>
              <input value={location} onChange={e => setLocation(e.target.value)} placeholder="e.g. Skopje, Macedonia" required />
            </div>
            <div className="form-group">
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  required
                  style={{ padding: '10px 14px', borderRadius: 8, border: '1px solid #ddd', fontSize: 14, width: '100%', colorScheme: 'light' }}
                />
                {dueDate && (
                  <div style={{
                    position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)',
                    background: 'white', pointerEvents: 'none', fontSize: 14, color: '#1a1a2e', paddingRight: 8
                  }}>
                    {`${dueDate.split('-')[2]}/${dueDate.split('-')[1]}/${dueDate.split('-')[0]}`}
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <div style={{ fontWeight: 700, fontSize: 16 }}>{t.materialsLabel} *</div>
            <button type="button" className="btn btn-dark" style={{ fontSize: 13 }} onClick={addMaterialItem}>+ {t.addMaterial}</button>
          </div>
          {materials.length === 0 ? (
            <div style={{ padding: '10px 14px', border: '1px dashed #ddd', borderRadius: 8, fontSize: 14, color: '#888' }}>
              {t.noMaterialsAvailable} <a href="/materials" style={{ color: '#1a1a2e', fontWeight: 600 }}>{t.materials}</a> {t.first}.
            </div>
          ) : (
              ...
              return (
                <div key={index} style={{ border: '1px solid #eee', borderRadius: 10, padding: 16, marginBottom: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                    <span style={{ fontWeight: 600, fontSize: 14 }}>{t.item} {index + 1}</span>
                  ...
                  <div className="form-group">
                    ...
                    <select value={item.materialId} onChange={e => updateMaterialItem(index, 'materialId', e.target.value)}
                      ...
                      {standardMaterials.length > 0 && <optgroup label={t.standardMaterials}>
                        {standardMaterials.map(m => <option key={m.id} value={m.id}>[{m.code}] {m.name}</option>)}
                      </optgroup>}
                      {handcraftMaterials.length > 0 && <optgroup label={t.handcraftMaterials}>
                    ...
                    <div className="form-group">
                      ...
                      {item.dimensions && item.dimensions !== 'x' && computed?.area > 0 && (
                        <span style={{ fontSize: 12, color: '#888', marginTop: 4 }}>
                          {t.calculated}: <strong>{computed?.area.toFixed(4)} m²</strong> | {t.cost}: <strong>{computed?.cost.toFixed(2)} EUR</strong>
                        </span>
                      )}
                    </div>
                  )}
                  {mat && mat.type === 'handcraft' && (
                    <div className="form-group">
                      ...
                      <span style={{ fontSize: 12, color: '#888' }}>{t.pricePerUnit}: <strong>{mat.price.toFixed(2)} EUR</strong> | {t.total}: <strong>{computed?.cost.toFixed(2)} EUR</strong></span>
                    </div>
                  )}
                </div>
              );
        ...
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ fontWeight: 700, fontSize: 16 }}>{t.additionalCosts}</div>
            <button type="button" className="btn btn-dark" style={{ fontSize: 13 }} onClick={addAdditionalCost}>+ {t.addCost}</button>
          </div>
          {additionalCosts.length === 0 && <p style={{ color: '#888', fontSize: 14 }}>{t.noAdditionalCosts}</p>}
          {additionalCosts.map((ac, index) => (
            <div key={index} style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 10 }}>
              <input type="number" min="0" step="0.01" placeholder={t.amountEur} value={ac.amount}
              ...
              <input placeholder={t.reasonCost} value={ac.reason}
        ...
        <div className="card">
          <div className="form-group" style={{ marginBottom: 0 }}>
            ...
            <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder={t.commentPlaceholder} rows={3} style={{ resize: 'vertical' }} />
          </div>
        </div>
        ...
        <div className="card">
          ...
          {additionalCosts.map((ac, index) => {
            ...
            return (
              <div key={`ac-${index}`} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, color: '#555', marginBottom: 6 }}>
                <span>{ac.reason || t.additionalCosts}</span>
          ...
          <div style={{ borderTop: '2px solid #1a1a2e', marginTop: 8, paddingTop: 10, display: 'flex', justifyContent: 'space-between', fontSize: 17, fontWeight: 700 }}>
            <span>{t.total}</span><span style={{ fontFamily: 'monospace' }}>{grandTotal.toFixed(2)} EUR</span>
          </div>
        </div>

        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 8, marginBottom: 40 }}>
          <button type="button" className="btn btn-gray" onClick={() => navigate('/orders')}>{t.cancel}</button>
          <button type="submit" className="btn btn-dark" disabled={!fullName.trim() || !hasValidItems}>{t.createOrderBtn}</button>
        </div>
      </form>
    </div>
  );
